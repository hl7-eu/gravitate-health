Instance: compositiontalzennajoar
InstanceOf: CompositionUvEpi
Title: "Composition for Package leaflet: Arab Gulf and Jordan Talzenna 0.25 mg, 1 mg capsules"
Description:  "Composition for Package leaflet: Arab Gulf and Jordan Talzenna 0.25 mg, 1 mg capsules"
Usage: #example

* identifier.system = "https://www.jordan.gov.jo/"
* identifier.value = "8481a3ac-a944-4d58-9c9e-3e102fea60da"

* status = #final

* type = https://www.jordan.gov.jo/#100000155538
* type.text = "Package Leaflet"

* date = "2020-03-01T00:00:00Z"
* author = Reference(orgpfizerinc)
* author[+] = Reference(orgexcellagmbh)
* author[+] = Reference(orgpfizerpharmceuticalsllc)

* title = "Arab Gulf and Jordan Talzenna® (talazoparib) 0.25 mg and 1mg hard capsules"
* language = #en

* contained
  * resourceType = "Binary"
  * id = "binarypfizerlogo"
  * contentType = #image/png
  * data = "" 

* extension.url = "http://www.jordan.gov.jo/fhir/extension/ValidationReference"
* extension
  * valueReference
    * reference = "#binarypfizerlogo"

* section[+]
  * title = "TITLE PAGE"
  * code = https://www.jordan.gov.jo/#100000155538
  * code.text = "Title Page"
  * text.status = #additional
  * text.div = """<div xmlns='http://www.w3.org/1999/xhtml'> 
<p style="text-align:center"><img src="#binarypfizerlogo" alt="Pfizer Logo"/></p>
<p dir="rtl" style="text-align:center"> تالزينا ®</p>
<p dir="rtl" style="text-align:center">تالازوباريب</p>
<p dir="rtl" style="text-align:center">كبسولات 0.25 مجم، 1 مجم</p>
<p dir="rtl" style="text-align:center">السوق المرجعية: الولايات المتحدة الأمريكية</p>
  </div>"""
 
* section[+]
  * title = "معلومات المريض"
  * code = https://www.jordan.gov.jo/#100000155538
  * code.text = "Package leaflet"
  * text.status = #additional
  * text.div = """<div xmlns='http://www.w3.org/1999/xhtml'> 
<p dir="rtl" style="text-align:center"><b>معلومات المريض</b></p>
        <p dir="rtl" style="text-align:center"><b>تالزينا</b>®<b> (تال-زين-ا)</b></p>
        <p dir="rtl" style="text-align:center"><b>(تالازوباريب)</b></p>
        <p dir="rtl" style="text-align:center"><b>كبسولات</b></p>
        <p dir="rtl"><b>ما هي أهم المعلومات التي ينبغي عليّ معرفتها عن تالزينا؟</b></p>
        <p dir="rtl"><b>يمكن أن يسبب تالزينا آثارًا جانبية خطيرة، بما في ذلك:</b></p>
        <p dir="rtl"><b>مشكلات نخاع العظم التي تعرف باسم متلازمة خلل التنسج النقوي (</b><b dir="ltr"
                >MDS</b><b>) أو ابيضاض الدم النقوي الحاد (</b><b dir="ltr">AML</b><b>).</b> بعض
            الأشخاص الذين يعانون من السرطان والذين تلقوا علاجًا سابقًا باستخدام العلاج الكيميائي أو
            بعض الأدوية الأخرى لعلاج السرطان لديهم أصيبوا بمتلازمة خلل التنسج النقوي أو ابيضاض الدم
            النقوي الحاد أثناء العلاج بتالزينا أو بعده. يمكن أن تؤدي متلازمة خلل التنسج النقوي أو
            ابيضاض الدم النقوي الحاد إلى الوفاة. إذا أصبت بمتلازمة خلل التنسج النقوي أو ابيضاض الدم
            النقوي الحاد، فسيقوم مقدم الرعاية الصحية الخاص بك بإيقاف العلاج بتالزينا.</p>
        <p dir="rtl">تُعد أعراض قلة تعداد خلايا الدم شائعة أثناء العلاج بتالزينا، ولكن من الممكن أن
            تكون علامة على وجود مشكلات خطيرة، بما في ذلك متلازمة خلل التنسج النقوي أو ابيضاض الدم
            النقوي الحاد. أخبر مقدم الرعاية الصحية الخاص بك إذا أصبت بأي من الأعراض التالية أثناء
            العلاج بتالزينا:</p>
        <table dir="rtl" border="1">
            <tbody>
                <tr>
                    <td>
                        <ul>
                            <li>الضعف</li>
                            <li>فقدان الوزن</li>
                            <li>الحمى</li>
                            <li>العدوى المتكررة</li>
                        </ul>
                    </td>
                    <td>
                        <ul>
                            <li>وجود دم في البول أو البراز</li>
                            <li>ضيق التنفس</li>
                            <li>الشعور بالتعب الشديد</li>
                            <li>التعرض للتكدم أو النزيف بسهولة أكبر</li>
                        </ul>
                    </td>
                </tr>
            </tbody>
        </table>
        <p dir="rtl">سيقوم مقدم الرعاية الصحية الخاص بك بإجراء فحوصات دم للتحقق من تعداد خلايا الدم لديك:</p>
        <ul dir="rtl">
            <li>قبل العلاج بتالزينا</li>
            <li>كل شهر أثناء العلاج بتالزينا</li>
            <li>أسبوعيًا إذا كنت تعاني من قلة تعداد خلايا الدم المستمرة لفترة طويلة. قد يقوم مقدم الرعاية الصحية الخاص بك بإيقاف العلاج بتالزينا إلى أن يتحسن تعداد خلايا الدم لديك.</li>
        </ul>
        <p dir="rtl">انظر "ما هي الآثار الجانبية المحتملة لتالزينا؟" أدناه لمعرفة الآثار الجانبية الأخرى لتالزينا.</p>
        <p dir="rtl"><b>ما هو تالزينا؟</b></p>
        <p dir="rtl">تالزينا هو دواء يصرف بوصفة طبية يُستخدم لعلاج البالغين الذين لديهم:</p>
        <ul dir="rtl">
            <li>نوع معين من سرطان الثدي (مستقبل عامل نمو البشرة البشري 2 [HER2]-سلبي)، و</li>
            <li>جين سرطان ثدي (BRCA) وراثي غير طبيعي، و</li>
            <li>سرطان انتشر إلى أجزاء أخرى من الجسم (متقدم موضعيًا أو منتشر). </li>
        </ul>
        <p dir="rtl"> سيقوم مقدم الرعاية الصحية الخاص بك بإجراء اختبار للتأكد من أن تالزينا مناسب
            لك.</p>
        <p dir="rtl">ليس معروفًا إذا ما كان تالزينا آمنًا وفعالًا في الأطفال أم لا.</p>
        <p></p>
        <p dir="rtl"><b>قبل تناول تالزينا، أخبر مقدم الرعاية الصحية الخاص بك بشأن جميع الحالات الطبية
                    التي تعاني منها، بما في ذلك إذا:</b></p>
        <ul dir="rtl">
            
                    <li>كنت تعاني من مشكلات في الكلى</li>
                    <li>كنتِ حاملًا أو تخططين للحمل. يمكن أن يُلحق تالزينا ضررًا بجنينكِ، وقد يسبب
                فقدان الحمل (الإجهاض). ينبغي ألا تصبحي حاملًا أثناء العلاج بتالزينا. أخبري مقدم
                الرعاية الصحية الخاص بكِ على الفور إذا كنتِ حاملًا أو أصبحتِ حاملًا أثناء العلاج
                بتالزينا. <ul>
                    <li>إذا كنتِ قادرة على الإنجاب، فقد يقوم مقدم الرعاية الصحية الخاص بكِ بإجراء
                        اختبار حمل قبل البدء في العلاج بتالزينا.</li>
                    <li><b>الإناث</b> القادرات على الإنجاب ينبغي أن يستخدمن وسيلة فعالة لتحديد النسل
                        (منع الحمل) أثناء العلاج بتالزينا ولمدة 7 أشهر على الأقل بعد تلقي الجرعة
                        الأخيرة من تالزينا. تحدثي إلى مقدم الرعاية الصحية الخاص بكِ بشأن أشكال منع
                        الحمل التي يمكن أن تكون مناسبة لك. </li>
                    <li><b>الذكور</b> الذين لديهم زوجات حوامل أو قادرات على الإنجاب ينبغي أن
                        يستخدموا وسيلة فعالة لمنع الحمل أثناء العلاج بتالزينا ولمدة 4 أشهر على الأقل
                        بعد تلقي الجرعة الأخيرة من تالزينا. </li>
                </ul></li>
                    <li>كنتِ تُرضعين رضاعة طبيعية أو تخططين لذلك. ليس معروفًا إذا ما كان تالزينا يمر
                        إلى لبن ثديكِ أم لا. لا تُرضعي رضاعة طبيعية أثناء العلاج بتالزينا ولمدة شهر
                        واحد على الأقل بعد تلقي الجرعة الأخيرة من تالزينا. تحدثي إلى مقدم الرعاية
                        الصحية الخاص بكِ بشأن أفضل طريقة لإرضاع طفلكِ أثناء هذه الفترة.</li>
        </ul>
        <p dir="rtl"><b>أخبر مقدم الرعاية الصحية الخاص بك بشأن جميع الأدوية التي تتناولها، </b>بما
            في ذلك الأدوية التي تصرف بوصفة طبية، والأدوية التي تصرف بدون وصفة طبية، والفيتامينات،
            والمكملات العشبية. من الممكن أن يؤثر تناول تالزينا مع بعض الأدوية الأخرى على كيفية عمل
            تالزينا وقد يسبب ذلك آثارًا جانبية. </p>
        <p dir="rtl">اعرف الأدوية التي تتناولها. احتفظ بقائمة بأدويتك واعرضها على مقدم الرعاية
            الصحية الخاص بك والصيدلي عند حصولك على دواء جديد. </p>
        <p dir="rtl"><b>كيف ينبغي عليّ تناول تالزينا؟</b></p>
        <ul dir="rtl">
            <li>تناول تالزينا بالضبط كما يخبرك مقدم الرعاية الصحية الخاص بك. </li>
            <li>لا تقم بتغيير جرعتك أو إيقاف تناول تالزينا دون التحدث أولًا مع مقدم الرعاية الصحية
                الخاص بك.</li>
            <li>تناول تالزينا مرة واحدة في اليوم.</li>
            <li>تناول تالزينا مع الطعام أو بدونه. </li>
            <li>ابتلع كبسولات تالزينا كاملة. لا تقم بإذابة كبسولات تالزينا أو فتحها. </li>
            <li>قد يقوم مقدم الرعاية الصحية الخاص بك بتغيير جرعتك من تالزينا أو قد يخبرك
                        بإيقاف تناول تالزينا بناءً على كيفية استجابتك للعلاج.</li>
            <li>إذا قمت بتفويت إحدى جرعات تالزينا أو تقيأت، فتناول جرعتك التالية في موعد
                        تناولك المعتاد لها. لا تتناول جرعة إضافية لتعويض جرعة فائتة.</li>
            <li>إذا تناولت كمية أكبر من اللازم من تالزينا، فاتصل بمقدم الرعاية الصحية الخاص
                        بك أو توجه إلى غرفة الطوارئ بأقرب مستشفى على الفور.</li>
        </ul>
        <p dir="rtl"><b>ما هي الآثار الجانبية المحتملة لتالزينا؟</b></p>
        <p dir="rtl"><b>يمكن أن يسبب تالزينا آثارًا جانبية خطيرة، بما في ذلك: </b></p>
        <p dir="rtl"><b>انظر "ما هي أهم المعلومات التي ينبغي عليّ معرفتها عن تالزينا؟"</b></p>
        <p dir="rtl"><b>تتضمن الآثار الجانبية الأكثر شيوعًا لتالزينا:</b></p>
        <table dir="rtl"  border="1">
            <tbody>
                <tr>
                    <td>
                        <ul>
                            <li>التعب أو الضعف</li>
                            <li>قلة عدد خلايا الدم البيضاء أو الحمراء</li>
                            <li>الغثيان</li>
                            <li>قلة عدد الصفائح الدموية</li>
                            <li>الصداع</li>
                            
                        </ul>
                                                
                    </td>
                    <td>
                        <ul>
                            <li>فقدان الشهية</li>
                            <li>الإسهال</li>
                            <li>القيء</li>
                            <li>فقدان الشعر </li>
                        </ul>                        
                    </td>
                </tr>
            </tbody>
        </table>
        <p dir="rtl">قد يؤدي تالزينا إلى مشكلات في الخصوبة لدى الذكور. قد يؤثر هذا على قدرتك على
            الإنجاب. تحدث إلى مقدم الرعاية الصحية الخاص بك إذا كان ذلك يقلقك. لا تقتصر الآثار
            الجانبية المحتملة لتالزينا على هذه الآثار. </p>
        <p dir="rtl">اتصل بطبيبك لتلقي المشورة الطبية حول الآثار الجانبية</p>
        <p dir="rtl"><b>كيف ينبغي عليّ تخزين تالزينا؟</b></p>
        <p dir="rtl">قم بتخزين تالزينا في درجة حرارة اقل من 30 درجة مئوية</p>
        <p dir="rtl"> شكل الجرعة والتركيزات:</p>
        <p dir="rtl"> زجاجة تحتوي على 30 كبسولة. التركيزات المتاحة هي 0.25 مجم و1 مجم</p>
        <p dir="rtl">
            <b>احفظ تالزينا وجميع الأدوية بعيدًا عن متناول الأطفال. </b></p>
        <p dir="rtl"><b>معلومات عامة حول الاستخدام الآمن والفعال لتالزينا.</b></p>
        <p dir="rtl">في بعض الأحيان توصف الأدوية لأغراض غير تلك المدرجة في نشرة معلومات المريض. لا
            تستخدم تالزينا لعلاج حالة لم يتم وصفه لها. لا تعطِ تالزينا لأشخاص آخرين، حتى إذا كانوا
            يعانون من نفس الأعراض التي تعاني منها. فقد يسبب لهم ضررًا. يمكنك أن تطلب من مقدم الرعاية
            الصحية الخاص بك أو الصيدلي تزويدك بالمعلومات الموجهة لأخصائيي الرعاية الصحية حول
            تالزينا. </p>
        <p style="text-align:right"><b>ما هي مكونات تالزينا؟</b></p>        
        <p dir="rtl"><b>المكون الفعال:</b> توزيلات تالازوباريب</p>
        <p dir="rtl"><b>المكونات غير الفعالة:</b> سليولوز بلوري مكروي سليسي (sMCC). تحتوي أغلفة
            الكبسولات غير الشفافة ذات اللونين الأبيض والعاجي، وذات اللونين الأبيض والأحمر الفاتح على
            هيبروميلوز (HPMC)، وأكسيد الحديد الأصفر، وأكسيد الحديد الأحمر، وثاني أكسيد التيتانيوم.
            يحتوي حبر الطباعة على صمغ اللك، وأكسيد الحديد الأسود، وهيدروكسيد البوتاسيوم، وهيدروكسيد
            الأمونيوم، والبروبيلين جليكول، وكحول بيوتيلي، وكحول الأيزوبروبيل، وكحول منزوع الماء</p>
        <p dir="rtl"> </p>
        <p dir="rtl"><b>مالك حق التسويق:</b></p>
        <p dir="rtl"></p>
        <p dir="ltr" style="text-align:right"><u>Pfizer Inc., 235 East 42nd street, </u></p>
        <p dir="ltr" style="text-align:right"><u>New York NY 10017-5755, </u></p>
        <p dir="ltr" style="text-align:right"><u>United States of America</u></p>
        <p dir="rtl"></p>
        <p dir="rtl"><b><u>التصنيع و التعبة الأولية</u></b></p>
        <p dir="rtl"> </p>
        <p dir="ltr" style="text-align:right"><u>Excella GmbH &amp; Co. KG </u></p>
        <p dir="ltr" style="text-align:right"><u>Nürnberger Strasse 12 </u></p>
        <p dir="ltr" style="text-align:right"><u>90537 Feucht </u></p>
        <p dir="ltr" style="text-align:right"><u>Germany</u></p>
        <p dir="rtl"></p>
        <p dir="rtl" style="text-align:right"><b><u>التعبئة الثانوية و الافراج النهائي:</u></b></p>
        <p dir="rtl"></p>
        <p dir="ltr" style="text-align:right"><u>Pfizer Pharmaceuticals LLC, </u></p>
        <p dir="ltr" style="text-align:right"><u>Km. 1.9 Road 689,Vega Baja, Puerto Rico 00693, </u></p>
        <p dir="ltr" style="text-align:right"><u>USA</u></p>
        <p dir="rtl"></p>
        <p dir="rtl"><b>تمت المراجعة الأخيرة لهذه النشرة في</b><b>مارس 2020</b></p>
        <p dir="rtl"> </p>
        <table>
            <tbody>
                <tr>
                    <td>
                        <p dir="rtl" style="text-align:center"><b>إن هذا دواء</b></p>
                        <ul dir="rtl">
                            <li>الدواء مستحضر يؤثر على صحتك واستهلاكه خلافًا للتعليمات يعرضك للخطر.</li>
                            <li>اتبع بدقة وصفة الطبيب وطريقة الاستخدام وتعليمات الصيدلي الذي قام بصرف الدواء.</li>
                            <li>الطبيب والصيدلي خبيران في الأدوية ومنافعها والمخاطر المترتبة على تناولها.</li>
                            <li>لا تقطع مدة العلاج المحددة لك من تلقاء نفسك.</li>
                            <li>لا تكرر صرف نفس الدواء بدون استشارة طبيبك.</li>
                        </ul>
                        <p dir="rtl" style="text-align:center"><b>تُحفظ جميع الأدوية بعيدًا عن متناول ومرأى الأطفال</b></p>
                        <p dir="rtl"><b>مجلس وزراء الصحة العرب</b></p>
                        <p dir="rtl"><b>اتحاد الصيادلة العرب</b></p>
                    </td>
                </tr>
            </tbody>
        </table>

  </div>""" 